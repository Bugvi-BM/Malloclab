/*
 * This solution uses an segregated free list. The amount of free lists can be adjusted
 * by changing the LISTLIMIT value. The segregated free lists splits blocks into "size classes"
 * which are defined by powers of two. Each individual free list uses the LIFO insertion policy.  
 * Finally, realloc has been optimised. It now checks if neighbouring blocks are free,
 * and utilises them if possible. We have observed that this method is significantly faster
 * than an explicit free list. However, there does not seem to be any difference in points awarded.
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
