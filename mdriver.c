...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    if (!fscanf(tracefile, "%d", &(trace->sugg_heapsize))) unix_error("fscanf failed"); /* not used */
    if (!fscanf(tracefile, "%d", &(trace->num_ids))) unix_error("fscanf failed");     
    if (!fscanf(tracefile, "%d", &(trace->num_ops))) unix_error("fscanf failed");     
    if (!fscanf(tracefile, "%d", &(trace->weight))) unix_error("fscanf failed");        /* not used */
	...
	switch(type[0]) {
	case 'a':
	    if (!fscanf(tracefile, "%u %u", &index, &size)) unix_error("fscanf failed");
	...
	case 'r':
	    ...
	    break;
	case 'f':
	    if(!fscanf(tracefile, "%ud", &index)) unix_error("fscanf failed");
